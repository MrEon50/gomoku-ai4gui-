...
class GomokuAI:
    def __init__(self, board_size=15):
        ...
        self.memory = deque(maxlen=200000)  # Increased experience memory
        self.opponent_memory = deque(maxlen=100000)  # Memory for opponent move prediction
        self.win_patterns = []  # Store winning patterns
        self.threat_patterns = []  # Store threat patterns
        
    def _build_model(self):
        # Input: 7x7 board pattern
        pattern_input = Input(shape=(7, 7, 3))
        
        # Convolutional layers with regularization and batch normalization
        x = Conv2D(64, (3, 3), padding='same', kernel_regularizer=l2(0.001))(pattern_input)
        ...
        x = Dense(256, kernel_regularizer=l2(0.001))(flat)
        x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = Dropout(0.3)(x)
        ...
        output = Dense(1, activation='tanh')(x)  # Pattern evaluation from -1 to 1
    ...
    def get_best_move(self, game):
        """Returns the best move based on pattern evaluation and strategic rules."""
    ...
    def remember(self, pattern, reward):
        """Remembers a pattern and its evaluation for later training."""
        self.memory.append((pattern, reward))
        
        # If this is a winning pattern with high reward, add to win_patterns
        if reward > 0.8:
            self.win_patterns.append((pattern, reward))
            # Keep only the best 1000 winning patterns
            if len(self.win_patterns) > 1000:
                self.win_patterns = sorted(self.win_patterns, key=lambda x: x[1], reverse=True)[:1000]
    ...
    def replay(self, batch_size=128, epochs=5):
        """Trains the model based on remembered experiences."""
        ...
        # Add some winning patterns to ensure they're learned well
        if self.win_patterns:
            win_samples = random.sample(self.win_patterns, min(len(self.win_patterns), batch_size // 4))
        ...
        # Train model
        history = self.model.fit(patterns, rewards, epochs=epochs, batch_size=min(64, batch_size), verbose=1)
        
        # Decay epsilon
        if self.epsilon > self.epsilon_min:
            self.epsilon *= self.epsilon_decay
        
        print(f"Model trained on {batch_size} patterns. Loss: {history.history['loss'][-1]:.4f}")
        
        # Train opponent prediction model if we have enough data
...
def train_initial_model(ai, epochs=10, num_patterns=10000, num_expert_patterns=5000):
    """Trains model based on generated patterns."""
    ...
    try:
        ...
        # Combine patterns
        all_patterns = np.concatenate([patterns, expert_patterns])
        all_labels = np.concatenate([labels, expert_labels])
...
def train_by_self_play(ai, num_games=20, board_size=15):
    ...
    for game_num in range(num_games):
        ...
        game.reset()
        game_patterns = []  # List of patterns from current game
        game_positions = []  # List of positions corresponding to patterns
        game_players = []    # List of players corresponding to patterns
        game_states = []     # List of board states for opponent prediction
